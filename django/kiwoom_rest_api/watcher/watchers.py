...
class KiwoomWatcher:
    ...
    async def run(self, main_func_name):
        ...
        if reqs:
            ...
            res = getattr(self.worker, main_func_name)(**req_content) # request 전송, response 수신
...
class TrWatcher(KiwoomWatcher):
    ...
    @property
    def req_dir(self):
        return 'tmp/tr/requests'
        #return os.path.join(settings.BASE_DIR, 'tmp/tr/requests')

    @property
    def res_dir(self):
        return 'tmp/tr/responses'
        #return os.path.join(settings.BASE_DIR, 'tmp/tr/responses')
...
class OrderWatcher(KiwoomWatcher):
    ...
    @property
    def req_dir(self):
        return 'tmp/order/requests'
        #return os.path.join(settings.BASE_DIR, 'tmp/order/requests')

    @property
    def res_dir(self):
        return 'tmp/order/responses'
        #return os.path.join(settings.BASE_DIR, 'tmp/order/responses')
